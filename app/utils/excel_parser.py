...
def leer_postulantes_desde_excel(archivo_excel):
    ...
    Se espera que los encabezados estén en la fila 5 del archivo (índice 4).
    """
    try:
        df = pd.read_excel(BytesIO(archivo_excel.read()), engine='openpyxl', header=4)
        ...
        for _, row in df.iterrows():
            postulante = Postulante(
                ...
                tipo_institucion=row.get("TIPO DE INSTITUCIÓN")
            )
